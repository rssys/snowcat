...
#define SKI_STATS_MAX_DATA_ADDRESSES 1*1000*1000*4
#define SKI_STATS_MAX_ACCESS_SIZE 9
#define SKI_STATS_MAX_DATA_INSTRUCTION_ADDRESSES 1*1000*1000*4
...
typedef struct struct_ski_stats_mem_access {
    unsigned int data_address;
    unsigned int ins_address;
    char cpu_no;
    char is_write;
    char num_bytes;
} ski_stats_mem_access;

typedef struct struct_ski_stats_pmc {
    ski_stats_mem_access cpu0_access;
    ski_stats_mem_access cpu1_access;
} ski_stats_pmc;
...
typedef struct struct_ski_stats_data_accesses{
	...
	ski_stats_instruction * cpu0_data_instructions_hash;
	ski_stats_instruction * cpu1_data_instructions_hash;
...
typedef struct struct_ski_stats{
    ...
    int cpu0_preemption_point;
    int cpu1_preemption_point;
    ...
    ski_ipfilter_hash_entry *ski_encountered_shared_ip_hash_by_cpu[2];
    ski_ipfilter_hash_entry ski_encountered_shared_ip_hash_entries_by_cpu[2][MAX_IPFILTER_HASH];
    int ski_num_encountered_shared_ip_by_cpu[2];

#define MAX_NUM_PMC 500000
    ski_stats_pmc pmc_list[MAX_NUM_PMC];
    int num_pmc;
...
//static inline void ski_stats_add_data_access_byte(unsigned int data_address, unsigned int eip_address, int is_write, unsigned int cpu_no, unsigned int value){
    ...
    if(da == 0){
		...
		da->cpu0_data_instructions_hash = 0;
		da->cpu1_data_instructions_hash = 0;
		da->total_accesses = 0;
...
*/
	...
	if (cpu_no == 0)
    	HASH_FIND_INT(da->cpu0_data_instructions_hash, &eip_address, di);
	else if (cpu_no == 1)
    	HASH_FIND_INT(da->cpu1_data_instructions_hash, &eip_address, di);
    ...
    if(di == 0){
		...
		if (cpu_no == 0)
			HASH_ADD_INT(da->cpu0_data_instructions_hash, eip_address, di);
		else if (cpu_no == 1)
			HASH_ADD_INT(da->cpu1_data_instructions_hash, eip_address, di);
       	//di->data_value_hash=0; 
		//HASH_ADD_INT(da->data_instructions_hash, eip_address, di);
    }
	else {
		// If the instruction can both write and read memory 
		if (is_write != di->is_write) {
#define INS_RW_MEM 2
#define INS_R_MEM 0
#define INS_W_MEM 1
			di->is_write = INS_RW_MEM;
		}
...
//static inline void ski_stats_add_data_access(unsigned int data_address, unsigned int eip_address, unsigned int size_bytes, int is_write, unsigned int cpu_no, unsigned int value){
	...
	ski_stats_add_data_access_byte(data_address, eip_address, is_write, cpu_no);
}
