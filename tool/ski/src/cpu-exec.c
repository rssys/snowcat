...
int ski_sched_cpu0_do_preemption = 0;
...
//    env->exception_index = -1;
    ...
    if (env->cpu_index == 0)
        env->ski_cpu.preemption_point = ski_init_options_preemptions_by_cpu[0];

    if (env->cpu_index == 1)
        env->ski_cpu.preemption_point = ski_init_options_preemptions_by_cpu[1];

    //printf("Initialize CPU-%d preemption-point to %d\n", env->cpu_index, env->ski_cpu.preemption_point);
...
void ski_threads_invert_priority(CPUState * env)
{
	...
	//SKI_TRACE("ski_threads_invert_priority: Num_thread %d Preemption point (EIP: %x cpu-%d) - changing priority of thread %d from %d to %d\n", ski_threads_no, env->eip, env->cpu_index, tc->id, tc->priority, new_priority);
	sprintf(debug_str, "ski_threads_invert_priority: Num_thread %d Preemption point (EIP: %x cpu-%d) - changing priority of thread %d from %d to %d compare_t %d", ski_threads_no, env->eip, env->cpu_index, tc->id, tc->priority, new_priority, compare_count);
	ski_exec_trace_print_comment(debug_str);	
	printf("ski_threads_invert_priority: Num_thread %d Preemption point (EIP: %x cpu-%d) - changing priority of thread %d from %d to %d compare_t %d\n", ski_threads_no, env->eip, env->cpu_index, tc->id, tc->priority, new_priority, compare_count);
