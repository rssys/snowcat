...
extern int ski_init_options_simplified_tracing;
extern int ski_sched_cpu1_access;
extern int ski_sched_cpu2_access;

extern int ski_sched_cpu0_do_preemption;
extern int ski_sched_cpu1_do_preemption;
extern int ski_sched_cpu2_do_preemption;
...
int ski_cpu1_preempt;
int ski_cpu2_preempt;
int ski_cpu1_preempt_count=0;
int ski_cpu2_preempt_count=0;
int snowboard_cpu1_preempt_count=0;
int snowboard_cpu2_preempt_count=0;
...
// c1000000 vmlinux.old
static int kernel_low =  3238002688;
// c22d3000 vmlinux.old
static int kernel_high = 0xc3000000;
static inline void dump_memory_access(uint32_t mem_address, uint32_t value, int mem_index, int n_bits, int is_load, FILE *fd)
{

	if(likely(ski_exec_trace_nr_entries < SKI_EXEC_TRACE_MAX_ENTRIES)){
		unsigned int mask;
		switch(n_bits){
		...
		if(ski_init_options_trace_memory_accesses_enabled && fd){
            if (!ski_init_options_simplified_tracing)
			    fprintf(fd,"### MEM: %d %x %x %x %x %d %c %x\n", (int) env->cpu_index, ski_last_eip, mem_address, value & mask,  mem_index, n_bits, is_load ? 'L' : 'S', paddr);
            else {
                // Only print out the memory access made by the kernel
                if (env->segs[R_CS].selector == __KERNEL_CS) {
                    if (env->cr[3] == env->ski_cpu.cr3) {
                        current_stack = ESP & 0XFFFFE000;
                        is_stack_mem = 0;
                        if (mem_address < current_stack || mem_address > current_stack + 0x2000)
                            is_stack_mem = 0;
                        else
                            is_stack_mem = 1;

                        fprintf(fd,"### MEM: %d %x %x %x %x %d %c %c %x\n", (int) env->cpu_index, ski_last_eip, mem_address, value & mask, mem_index, n_bits, is_load ? 'L' : 'S', is_stack_mem ? 'L' : 'G', paddr);
                    }
                }
            }
        }
        ...
        if(ski_forkall_enabled){
            if (ski_init_options_preemption_by_eip){
                if (env->segs[R_CS].selector == __KERNEL_CS)
                    if (env->cr[3] == env->ski_cpu.cr3) {
                        if ((int)env->cpu_index <= 1) {
                            current_stack = ESP & 0XFFFFE000;
                            // Only monitor kernel memory accesses to non-stack memory
                            if (mem_address < current_stack || mem_address > current_stack + 0x2000)
...
void helper_ski_load_access(uint32_t address, uint32_t value, int mem_index, int n_bits) {
    ...
    if (ski_init_options_preemption_by_eip){
        ski_pmc_preemption_check(address, value, n_bits);
    }

    if (ski_init_options_preemption_by_access){
        ski_preemption_point_check(address, value, n_bits);
    }
...
int cpu1_hint_addr, cpu1_hint_ins;
int cpu2_hint_addr, cpu2_hint_ins;
int update_value = -1;
extern int ski_cpu1_hint_addr, ski_cpu1_hint_ins;
extern int ski_cpu2_hint_addr, ski_cpu2_hint_ins;

extern ski_stats_pmc preemption_pmc;
void ski_pmc_preemption_check(uint32_t address, uint32_t value, int n_bits) {
    if  (preemption_pmc.cpu0_access.ins_address == -1 ||
    ...
    if (env->cpu_index == 0) {
        if (ski_stats_get_self()->ski_cpu0_accessed < 2 &&
                address == preemption_pmc.cpu0_access.data_address &&
                ski_last_eip == preemption_pmc.cpu0_access.ins_address) {
            ski_sched_cpu0_do_preemption = 1;
            ski_stats_get_self()->ski_cpu0_accessed++;
        }
    } else {
        if (ski_stats_get_self()->ski_cpu1_accessed < 2 &&
                address == preemption_pmc.cpu1_access.data_address &&
                ski_last_eip == preemption_pmc.cpu1_access.ins_address) {
            ski_sched_cpu1_do_preemption = 1;
            ski_stats_get_self()->ski_cpu1_accessed++;
        }
    }
...
// This function is only called when ski_init_options_preemption_by_access is enabled, which implies concurrent mode
void ski_preemption_point_check(uint32_t address, uint32_t value, int n_bits){
    int n_bytes = 0;
    switch(n_bits){
			case 8: n_bytes = 1; break;
			case 16: n_bytes = 2; break;
			case 32: n_bytes = 4; break;
			default: assert(0); break;
	}
    // If this memory access fits the scheduling hint, set preemption flag
    if (env->cpu_index == 0){
        if ((ski_last_eip == ski_cpu1_hint_ins && address == ski_cpu1_hint_addr) ||
            (ski_last_eip == ski_init_cpu1_preemption && (address<=ski_init_preemption_channel_addr && ski_init_preemption_channel_addr < address + n_bytes)
			 && (ski_init_cpu1_preemption_value == -2 || ski_init_cpu1_preemption_value == value) )){
				ski_sched_cpu1_do_preemption = 1;
        }
    }
    else if (env->cpu_index == 1){
        if ((address == ski_cpu2_hint_addr && ski_last_eip == ski_cpu2_hint_ins) ||
            (ski_last_eip == ski_init_cpu2_preemption && (address<=ski_init_preemption_channel_addr && ski_init_preemption_channel_addr < address + n_bytes)
			 && (ski_init_cpu2_preemption_value == -2 || ski_init_cpu2_preemption_value == value) )){
				ski_sched_cpu2_do_preemption = 1;
        }
    }

    if (address<=ski_init_preemption_channel_addr && ski_init_preemption_channel_addr < address + n_bytes){
        int random_seed = -1;
        if (env->cpu_index == 0){
            if (!cpu0_found && ski_last_eip == ski_init_cpu1_preemption){
                // only update ski_stats with the first addr, maybe also the second
                cpu0_found = 1;
                update_value = value;
                ski_stats_get_self()->ski_cpu1_hint_addr = cpu1_hint_addr;
                ski_stats_get_self()->ski_cpu1_hint_ins = cpu1_hint_ins;
            }
            random_seed = random() % 2;
            if (random_seed ==0){
                ski_sched_cpu1_do_preemption = 1;
            }
        }
            ...
            if (env->cpu_index == 1){
                if (!cpu1_found && ski_last_eip == ski_init_cpu2_preemption){
                        cpu1_found = 1;
                        ski_stats_get_self()->ski_cpu2_hint_addr = cpu2_hint_addr;
                        ski_stats_get_self()->ski_cpu2_hint_ins = cpu2_hint_ins;
                        if (cpu0_found){
                            if (value == update_value)
                                ski_stats_get_self()->communicate = 1;
                        }
                }
                random_seed = random() % 2;
                if (random_seed ==0){
    ...
    if (env->cpu_index == 0){
        cpu1_hint_addr = address;
        cpu1_hint_ins = ski_last_eip;
    }
    else if (env->cpu_index == 1){
        cpu2_hint_addr = address;
        cpu2_hint_ins = ski_last_eip;
    }
}
...
int cpu0_nr_instr_executed = -1;
int cpu1_nr_instr_executed = -1;
void helper_ski_instrument_possible_inversion_point(int explicit_eip)
{
	if(env->ski_active == false){
		...
		if(unlikely(ski_correct_thread()==false)){
            ...
            if (ski_sched_cpu0_do_preemption == 1 && env->cpu_index == 0) {
                do_preemption = 1;
                ski_sched_cpu0_do_preemption = 0;
            }

            if (ski_sched_cpu1_do_preemption == 1 && env->cpu_index == 1) {
                do_preemption = 1;
                ski_sched_cpu1_do_preemption = 0;
            }

            if(unlikely(do_preemption)){
				EIP = explicit_eip;
                // This function might be for syncing all CPU's states
				cpu_synchronize_all_states();
                // Sishuai reading 
                // this function mainly prints loop information
				ski_loop_flush();
                // Mark the current CPU as preempted
                if (env->cpu_index == 0){
                    ski_cpu1_preempt = 1;
                }
                else if (env->cpu_index == 1){
				...
				if (!ski_init_options_preemption_by_eip){
					...
					while (penv) {
						if(penv->ski_active == true){
							...
							printf("[Reschedule-point-change] Changing CPU-%d reschedule point to %d\n",
									penv->cpu_index, penv->ski_cpu.nr_max_instr);
						}
...
static inline void ski_exec_trace_print_entry(FILE *fd, int explicit_eip){
	...
	if(ski_init_options_trace_instructions_enabled){
        if (!ski_init_options_simplified_tracing)
            ...
            // Only print the instruction address if it comes from the kernel
            if (env->segs[R_CS].selector == __KERNEL_CS)
                if (env->cr[3] == env->ski_cpu.cr3)
                    fprintf(fd, "%d %x\n",
                        (int) env->cpu_index,
		...
		if(ski_exec_trace_execution_fd && (ski_exec_trace_nr_entries < SKI_EXEC_TRACE_MAX_ENTRIES)){
			...
			if(ski_forkall_enabled){
                if (env->segs[R_CS].selector == __KERNEL_CS) {
                    if (env->cr[3] == env->ski_cpu.cr3) {
                        if (!ski_init_options_preemption_by_access){
                            ski_stats_add_instruction_access(explicit_eip, env->cpu_index);
                        }
                    }
                    // Sishuai to-do
                    // I don't think we have to do stats here
                    //else{
                    //    if ((env->cpu_index==0 && explicit_eip== ski_init_cpu1_preemption) || (env->cpu_index==1 && explicit_eip==ski_init_cpu2_preemption)){
				    //        ski_stats_add_instruction_access(explicit_eip, env->cpu_index);
                    //    }
                    //}
                }
    ...
    if ((existing_pos >= 0) && (ski_loop_equal_vcpu_regs(existing_pos, explicit_eip)==1)){
        ...
        if (new_pos == (old_pos+1)%SKI_LOOP_MAX_LOG_ENTRIES){
            ...
            if (ski_loop_log[new_pos].count >= SKI_LOOP_INTERATIONS_ACTIVATE){
				ski_loop_store_instruction(&ski_loop_log[new_pos],explicit_eip, 0);
				if(ski_init_options_trace_instructions_enabled && ski_loop_log[new_pos].count % SKI_DEBUG_LOOP_CYCLE_IN_PROGREESS == 0){
					char debug[128];
