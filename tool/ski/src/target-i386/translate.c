...
enum {
	...
	#define SKI_INTERCEPT_QEMU_MEM_TRACE(function, is_load, bits)												\
		 static inline void ski_##function(TCGv ret, TCGv addr, int mem_index){								\
			...
			if(is_load){																						\
				function(ret,addr,mem_index);																	\
				if(likely(ski_exec_trace_execution_fd || ski_mem_read_fd)){													\
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
    ...
    int rex_w, rex_r;
    // SKI-related variables
    ski_ipfilter_hash_entry *e;
    int eip;

    /*
     * Check if the next instruction is in the ipfilter.
     * If so, that means this instruction is likely to access some
     * shared memory, so we will try to make schedule decisions.
     */
    if (s->cc_op != CC_OP_DYNAMIC)
        gen_op_set_cc_op(s->cc_op);
